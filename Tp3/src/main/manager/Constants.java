/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package main.manager;

import java.awt.Graphics;

/**
 *
 * @author Agus_
 */
public class Constants {
    public static final int SIDE_SPRITE=32;
    public static int WINDOW_WIDTH = 800;
    public static int WINDOW_HEIGHT = 600;
    public static int UFS = 0;
    public static int WINDOW_CENTER_X = WINDOW_WIDTH / 2;
    public static int WINDOW_CENTER_Y = WINDOW_HEIGHT / 2;
    
    public static boolean BATTLE = false;
    
    public static int SELECTED_CHARACTER = 1;
    
    public static String RUTA_ENEMIGOS = "textures/spriteSheetE.png";
    
    public static String RUTA_MAP = "resources/text/mapTemplate.txt";
    public static String RUTA_MAGOELFO = "textures/magoElfo.png";
    public static String RUTA_MAGOGNOMO = "textures/MagoGnomo.png";
    public static String RUTA_GUERREROHUMANO= "textures/GuerreroHumano.png";
    public static String RUTA_GUERREROENANO=  "textures/GuerreroEnano.png";
    
    public static String RUTA_BATALLA = "textures/batalla.png";
    public static String RUTA_GNOMO = "textures/Gnomo.png";
    public static String RUTA_ELFO = "textures/Elfo.png";
    public static String RUTA_HUMANO = "textures/Humano.png";
    public static String RUTA_ENANO = "textures/Enano.png";
    public static String RUTA_GNOMOE = "textures/GnomoE.png";
    public static String RUTA_ELFOE = "textures/ElfoE.png";
    public static String RUTA_HUMANOE = "textures/HumanoE.png";
    public static String RUTA_ENANOE = "textures/EnanoE.png";
    
    public static boolean NEWGAME = false;
    public static int ACTUAL_ENEMY_ZONE = 0;
    public static int ACTUAL_MAP = 0;
    
    public static boolean dead = false;
    
    public static int PRUEBA = 0;
    
    public static boolean EN_BATALLA = false;
    public static boolean ESC = true;
    
    public static boolean BATTLESTATE = false;
    
    
    public static boolean h1 = false;
    public static boolean h2 = false;
    public static boolean h3 = false;
    public static boolean h4 = false;
    
    
    public static String RUTA_LAYOUT = "textures/filtro.png";
}