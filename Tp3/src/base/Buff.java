...
public class Buff extends Status {
    ...
    public Buff() {
        ...
        critB = 0;
        defB = 0;
        active = false;//se inicializa en false porque al momento de aplicarse sobre un obvjetivo se clona este objeto y se lo pasa al objetivo, necesitando que este este en "false" porque es un buff "nuevo"
        //se ve mejor en la funcion, NUNCA tiene que inicializarse en true
        buffed=false;
    }
    ...
    //el constructor de clon es CLAVE para que ande bien este programa, si no pasamos un clon lo que sucederia es que a la hora de ir restandole duracion al status de un objetivo, le estariamos restando la duracion al objeto en si de la habilidad que lo uso, haciendo que permanentemente quede en 0 la duracion
   ...
   accion de la afliccion en un turno, y resta duracion en el proceso*/
    public String statusTurn(Character c) {
        ...
        if (getDuration() > 0) { //si todavia tiene duracion disponible surge el efecto del status
            ...
            if (!active) { /*aca es donde entra en juego el boolean de la clase, si es false quiere decir que 
